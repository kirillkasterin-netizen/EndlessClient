...
@ModuleAnnotation(name = "Cosmetics", description = "Визуальные украшения", category = Category.RENDER)
public class Cosmetics extends Module {
    ...
    private final MultiBooleanSetting cosmetics = MultiBooleanSetting.create("Косметика", List.of(
            "Нимб", "Крылья", "Крылья 2"
    ));

    private final BooleanSetting butterflyWingAnimation = new BooleanSetting("Анимация крыльев", true, () -> cosmetics.isEnable("Крылья"));
    private final NumberSetting butterflyWingSize = new NumberSetting("Размер Крыльев", 1.0f, 0.65f, 1.8f, 0.05f, () -> cosmetics.isEnable("Крылья"));

    private final BooleanSetting classicWingAnimation = new BooleanSetting("Анимация Крыльев 2", true, () -> cosmetics.isEnable("Крылья 2"));
    private final NumberSetting classicWingSize = new NumberSetting("Размер Крыльев 2", 1.0f, 0.65f, 1.8f, 0.05f, () -> cosmetics.isEnable("Крылья 2"));
    ...
    public void onRender3D(EventRender3D event) {
        ...
        if (cosmetics.isEnable("Нимб")) renderNimbus(event);
    ...
    private void renderButterflyWings(PlayerEntity player, float tickDelta, MatrixStack matrices, Vec3d cameraPos) {
        ...
        int topColor     = setAlpha(theme,           132);
        int bottomColor  = setAlpha(darken(theme, 0.85f), 102);
        int outlineColor = setAlpha(darken(theme, 0.58f), 214);
    ...
    private void renderClassicWings(PlayerEntity player, float tickDelta, MatrixStack matrices, Vec3d cameraPos) {
        ...
        int theme        = resolveCosmeticThemeColor();
        int baseColor    = setAlpha(theme, CLASSIC_WING_DEFAULT_ALPHA);
        int glowColor    = setAlpha(interpolateColor(theme, 0xFFFFFFFF, 0.28f), Math.round(CLASSIC_WING_DEFAULT_ALPHA * 0.22f));
        int coreColor    = setAlpha(interpolateColor(theme, 0xFFFFFFFF, 0.55f), Math.round(CLASSIC_WING_DEFAULT_ALPHA * 0.26f));
        int outlineColor = setAlpha(darken(theme, 0.62f),                                    Math.round(CLASSIC_WING_DEFAULT_ALPHA * 0.62f));
        int ribsColor    = setAlpha(interpolateColor(theme, 0xFFFFFFFF, 0.28f), Math.round(CLASSIC_WING_DEFAULT_ALPHA * 0.20f));
    ...
    private void renderNimbus(EventRender3D event) {
        ...
        for (int arm = 0; arm < NIMBUS_ARMS; arm++) {
            ...
            for (int segment = 0; segment < NIMBUS_SEGMENTS; segment++) {
                ...
                int   color    = setAlpha(baseColor, alpha);
    ...
    private void renderNimbusBillboard(MatrixStack matrices, float cameraYaw, float cameraPitch,
        ...
        // Круглый билборд - triangle fan с softer edge для сглаживания
        int segments = 24;
        ...
        // Край - 0 альфа для размытого края (sub-pixel anti-aliasing)
        for (int i = 0; i <= segments; i++) {
            double angle = (double) i / segments * Math.PI * 2.0;
            float cosA = (float) Math.cos(angle);
            float sinA = (float) Math.sin(angle);
            buffer.vertex(matrix, cosA * radius, sinA * radius, 0f).color(r, g, b, 0);
        }
        BufferRenderer.drawWithGlobalProgram(buffer.end());

        // Внутренний более плотный диск для яркого ядра
        float coreRadius = radius * 0.55f;
        BufferBuilder core = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        core.vertex(matrix, 0f, 0f, 0f).color(r, g, b, a);
        for (int i = 0; i <= segments; i++) {
            double angle = (double) i / segments * Math.PI * 2.0;
            float cosA = (float) Math.cos(angle);
            float sinA = (float) Math.sin(angle);
            core.vertex(matrix, cosA * coreRadius, sinA * coreRadius, 0f).color(r, g, b, a);
        }
    ...
    private boolean shouldRenderCosmeticForPlayer(PlayerEntity player) {
        ...
        return Endless.getInstance().getFriendManager().isFriend(player.getName().getString());
    }
    ...
    private void syncWingSelectionState() {
        boolean butterfly = cosmetics.isEnable("Крылья");
        boolean classic   = cosmetics.isEnable("Крылья 2");
        if (butterfly && classic) {
            if (butterfly != lastButterflySelected && classic == lastClassicSelected) {
                cosmetics.getValueByName("Крылья 2").setEnabled(false);
                classic = false;
            } else {
                cosmetics.getValueByName("Крылья").setEnabled(false);
                butterfly = false;
            }
        }
    ...
    private void renderClassicWingSide(MatrixStack matrices, float side, float open, float sidePitch, float sideRoll,
        ...
        RenderSystem.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE);
        drawClassicWingLayer(matrices, side, 1.22f, glowColor, setAlpha(glowColor, 0));
        drawClassicWingLayer(matrices, side, 0.84f, coreColor, setAlpha(coreColor, 0));

        RenderSystem.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        drawClassicWingLayer(matrices, side, 1.0f, baseColor, setAlpha(baseColor, 10));
    ...
    private void drawClassicWingRibs(MatrixStack matrices, float side, float scale, int color) {
        ...
        for (int ribIndex : ribIndices) {
            ...
            vertex(buffer, matrix, 0.0f, 0.0f, 0.0f,
                    setAlpha(color, Math.max(8, (int) (((color >> 24) & 0xFF) * 0.75f))));
    ...
    private int applyClassicWingPointAlpha(int color, float multiplier) {
        ...
        return setAlpha(color, Math.max(0, Math.min(255, (int) (alpha * multiplier))));
    }
