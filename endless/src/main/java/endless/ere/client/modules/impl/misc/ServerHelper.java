...
public final class ServerHelper extends Module {
    ...
    private final BooleanSetting consumablesSetting = new BooleanSetting("Таймер расходников", true,()->Endless.getInstance().getServerHandler().isCopyTime()||Endless.getInstance().getServerHandler().isHolyWorld());

    private final BooleanSetting autoPointSetting = new BooleanSetting("Авто точка", true, Endless.getInstance().getServerHandler()::isFunTime);
    ...
    public void initialize() {
        ...
        keyBindings.add(new KeyBind(Items.FIREWORK_STAR,
                new KeySetting("Анти флай", Endless.getInstance().getServerHandler()::isReallyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FLOWER_BANNER_PATTERN,
                new KeySetting("Опыт прокрутки", Endless.getInstance().getServerHandler()::isReallyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.PRISMARINE_SHARD,
                new KeySetting("Взрывная трапка", Endless.getInstance().getServerHandler()::isHolyWorld),
                5, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.POPPED_CHORUS_FRUIT,
                new KeySetting("Обыч трапка", Endless.getInstance().getServerHandler()::isHolyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.NETHER_STAR,
                new KeySetting("Стан", Endless.getInstance().getServerHandler()::isHolyWorld),
                30, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FIRE_CHARGE,
                new KeySetting("Взрывная штука", Endless.getInstance().getServerHandler()::isHolyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.SNOWBALL,
                new KeySetting("Снежок", () -> Endless.getInstance().getServerHandler().isCopyTime()
                        || Endless.getInstance().getServerHandler().isHolyWorld()),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.PHANTOM_MEMBRANE,
                new KeySetting("Божья аура", Endless.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.NETHERITE_SCRAP,
                new KeySetting("Трапка", Endless.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.DRIED_KELP,
                new KeySetting("Пласт", Endless.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.SUGAR,
                new KeySetting("Явная пыль", Endless.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FIRE_CHARGE,
                new KeySetting("Огненный смерч", Endless.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.ENDER_EYE,
                new KeySetting("Дезорент", Endless.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));
    ...
    public void onPacket(EventPacket e) {
        ...
        if(this.consumablesSetting.isEnabled()){
            if ( Endless.getInstance().getServerHandler().isCopyTime()&&e.getPacket() instanceof ChunkDeltaUpdateS2CPacket chunkDelta) {
            ...
            if (e.getPacket() instanceof PlaySoundS2CPacket soundS2CPacket) {
                if (Endless.getInstance().getServerHandler().isHolyWorld() && soundS2CPacket.getSound().toString().equals("Reference{ResourceKey[minecraft:sound_event / minecraft:block.beacon.deactivate]=SoundEvent[location=minecraft:block.beacon.deactivate, fixedRange=Optional.empty]}")) {
            ...
            if (Endless.getInstance().getServerHandler().isHolyWorld() && e.getPacket() instanceof ParticleS2CPacket particleS2CPacket) {
        ...
        keyBindings.stream().filter(bind -> bind.draw.isValue()).forEach(bind -> {
            ...
            switch (bind.setting.getName()) {
                case "Трапка", "Обыч трапка" ->
                        drawItemCube(playerPos, smooth, 1.99F, Endless.getInstance().getThemeManager().getClientColor(90).getRGB());
                case "Дезорент", "Огненный смерч", "Явная пыль" ->
                        drawItemRadius(matrix, bind.distance, ColorUtil.LIGHT_RED);
                case "Взрывная штука" ->
                        drawItemRadius(matrix, 5, Endless.getInstance().getThemeManager().getClientColor(90).getRGB());
                case "Пласт" -> {
                    ...
                    if (Math.abs(mc.player.getPitch()) > 60) {
                        ...
                        Render3DUtil.drawBox(new Box(pos1, pos2), Endless.getInstance().getThemeManager().getClientColor(90).getRGB(), 3, true, true, true);
                        ...
                        drawSidePlast(playerPos.east(5).south().down(), smooth, Endless.getInstance().getThemeManager().getClientColor(90).getRGB(), -1, true);
                    } else if (yaw <= -67.5F) {
                        ...
                        drawSidePlast(playerPos.east(5).down(), smooth, Endless.getInstance().getThemeManager().getClientColor(90).getRGB(), 1, false);
                        ...
                        drawSidePlast(playerPos.west(4).down(), smooth, Endless.getInstance().getThemeManager().getClientColor(90).getRGB(), 1, true);
                    } else if (yaw <= 112.5F) {
                        ...
                        drawSidePlast(playerPos.west(4).south().down(), smooth, Endless.getInstance().getThemeManager().getClientColor(90).getRGB(), -1, false);
                    }
                }
    ...
    public void onDraw(EventRender2D e) {
        ...
        structures.forEach(cons -> {
            ...
            if (ProjectionUtil.canSee(cons.vec) && cons.anarchy == Endless.getInstance().getServerHandler().getAnarchy() && Endless.getInstance().getServerHandler().getWorldType().equals(cons.world)) {
...
//                        .round(1.5F).color(ColorUtil.HALF_BLACK).build());
                ...
                e.getContext().drawRoundedRect(posX - 4, posY - 4, (16 * 0.8f + 4 + font.width(text) + 8), 16 * 0.8f + 8, BorderRadius.all(4), Endless.getInstance().getThemeManager().getCurrentTheme().getForegroundLight());
                DrawUtil.drawRoundedCorner(e.getContext().getMatrices(),posX - 4, posY - 4, (16 * 0.8f + 4 + font.width(text) + 8), 16 * 0.8f + 8,0.1f,10,Endless.getInstance().getThemeManager().getCurrentTheme().getColor(),BorderRadius.all(4));

                e.getContext().drawText(font, text, posX + 16 * 0.8f + 4, posY + 2.5f, Endless.getInstance().getThemeManager().getCurrentTheme().getColor());
        ...
        serverEvents.forEach(event -> {
            ...
            if (ProjectionUtil.canSee(event.vec) && event.anarchy == Endless.getInstance().getServerHandler().getAnarchy() && Endless.getInstance().getServerHandler().getWorldType().equals(event.world)) {
    ...
    private void drawItemCube(BlockPos playerPos, Vec3d smooth, float size, int color) {
        ...
        boolean inBox = mc.world.getPlayers().stream().anyMatch(ent -> ent != mc.player && box.intersects(ent.getBoundingBox()) && !Endless.getInstance().getFriendManager().isFriend(ent.getGameProfile().getName()));
        Render3DUtil.drawBox(box, inBox ? Endless.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB() : color, 3, true, true, true);
    }

    private void drawItemRadius(MatrixStack matrix, float distance, int clr) {
        float playerHalfWidth = mc.player.getWidth() / 2;
        int color = validDistance(distance) ? Endless.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB() : clr;
    ...
    private void addEvent(String name, String lvl, String owner, Vec3d vec3d, String world, int timeOpen, int timeLoot) {
        if (serverEvents.stream().noneMatch(server -> server.vec.equals(vec3d))) {
            ...
            serverEvents.add(new ServerEvent(name, lvl, owner, vec3d, world, Endless.getInstance().getServerHandler().getAnarchy(), open, loot));
        }
    }

    private void addStructure(Item item, Vec3d vec, double time) {
        if (structures.stream().noneMatch(str -> str.vec.equals(vec))) {
            structures.add(new Structure(item, vec, Endless.getInstance().getServerHandler().getWorldType(), Endless.getInstance().getServerHandler().getAnarchy(), time));
        }
    }
    ...
    private boolean validDistance(float dist) {
        return dist == 0 || mc.world.getPlayers().stream().anyMatch(p -> p != mc.player && !Endless.getInstance().getFriendManager().isFriend(p.getGameProfile().getName()) && mc.player.distanceTo(p) <= dist);
    }
