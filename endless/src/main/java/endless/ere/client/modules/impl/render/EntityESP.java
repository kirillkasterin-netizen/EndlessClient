...
import endless.ere.client.modules.impl.misc.NameProtect;
...
import endless.ere.utility.mixin.client.render.LivingEntityRendererMixin;
...
public final class EntityESP extends Module {
    ...
    private final ModeSetting.Value endless = new ModeSetting.Value(textMode, "Endless").select();
    ...
    public void onHudRender(EventRender3D event) {
        ...
        for (Entity ent : mc.world.getPlayers()) {
            ...
            if (ent instanceof PlayerEntity entity) {
                ...
                Render3DUtil.drawBox((entity.getBoundingBox().offset(MathUtil.interpolate(entity).subtract(entity.getPos()))), Endless.getInstance().getThemeManager().getClientColor(180).getRGB(), 1f);
            }
        }
    }
    ...
    private void drawHands(CustomDrawContext context, PlayerEntity player, Vector4d vec) {
        ...
        for (ItemStack stack : player.getHandItems()) {
            ...
            if(text.getSiblings().isEmpty()) {
                text = text.copy().setStyle(Style.EMPTY.withColor(Endless.getInstance().getThemeManager().getCurrentTheme().getWhite().getRGB()));
            }
            ...
            if (stack.getCount() > 1) {
                text = text.copy().append(Text.of(" x" + stack.getCount()).copy().setStyle(Style.EMPTY.withColor(Endless.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB())));
                ;
            }
            ...
            ColorRGBA rectColor = Endless.getInstance().getFriendManager().isFriend(player.getGameProfile().getName())
                    ? theme.getColor().withAlpha(theme.getForegroundLight().getAlpha())
                    : new ColorRGBA(10, 10, 12, 215);
            ...
            DrawUtil.drawBlur(context.getMatrices(), rectX, (float) posY, rectWidth, rectHeight, 18f, BorderRadius.all(2), ColorRGBA.WHITE);

            context.drawRoundedRect(rectX, (float) posY, rectWidth, rectHeight, BorderRadius.all(2), rectColor);
            DrawUtil.drawHudDots(context.getMatrices(), rectX, (float) posY, rectWidth, rectHeight, BorderRadius.all(2), 0.16f);
    ...
    private void renderNameTag(CustomDrawContext context, PlayerEntity player, float tickDelta) {
        ...
        if (mojo.isSelected()) {
            Text playerName = (player == mc.player ||
                    (NameProtect.getCustomName() != null && Endless.getInstance().getFriendManager().isFriend(player.getNameForScoreboard())))
            ...
            DrawUtil.drawBlur(context.getMatrices(), rectX, rectY, rectWidth, rectHeight, 18f, BorderRadius.all(2), ColorRGBA.WHITE);

            context.drawRoundedRect(rectX, rectY, rectWidth, rectHeight, BorderRadius.all(2), rectColor);
            DrawUtil.drawHudDots(context.getMatrices(), rectX, rectY, rectWidth, rectHeight, BorderRadius.all(2), 0.16f);
            ...
            DrawUtil.drawBlur(context.getMatrices(), rectX2, rectY2, rectWidth2, rectHeight2, 18f, BorderRadius.all(2), ColorRGBA.WHITE);

            context.drawRoundedRect(rectX2, rectY2, rectWidth2, rectHeight2, BorderRadius.all(2), rectColor);
            DrawUtil.drawHudDots(context.getMatrices(), rectX2, rectY2, rectWidth2, rectHeight2, BorderRadius.all(2), 0.16f);
        ...
        if ( this.elements.isEnable(2)) {
            ...
            if (nonEmpty > 0) {
                ...
                DrawUtil.drawBlur(context.getMatrices(), rectX, rectY, totalWidth, rectHeight, 18f, BorderRadius.all(2), ColorRGBA.WHITE);
                context.drawRoundedRect(rectX, rectY, totalWidth, rectHeight, BorderRadius.all(2), rectColor);
                DrawUtil.drawHudDots(context.getMatrices(), rectX, rectY, totalWidth, rectHeight, BorderRadius.all(2), 0.14f);
    ...
    private void renderItemESP(CustomDrawContext context, ItemEntity ent, float tickDelta) {
        ...
        if(label.getSiblings().isEmpty()) {
            label = label.copy().setStyle(Style.EMPTY.withColor(Endless.getInstance().getThemeManager().getCurrentTheme().getWhite().getRGB()));
        }
        ...
        if (ent.getStack().getCount() > 1) {
            label = label.copy().append(Text.of(" x" + ent.getStack().getCount()).copy().setStyle(Style.EMPTY.withColor(Endless.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB())));
            ;
        }
        ...
        ColorRGBA rectColor = new ColorRGBA(10, 10, 12, 215);
        context.drawRoundedRect(rectX, rectY, rectW, rectH, BorderRadius.all(2), rectColor);
        DrawUtil.drawHudDots(context.getMatrices(), rectX, rectY, rectW, rectH, BorderRadius.all(2), 0.16f);
    ...
    private void renderEntityBox(CustomDrawContext context, Entity entity, float tickDelta) {
        ...
        drawFlatBox((entity instanceof PlayerEntity player && Endless.getInstance().getFriendManager().isFriend(player.getGameProfile().getName())), vec4d);

    }

    private void drawFlatBox(boolean friend, Vector4d vec) {
        int client1 = !friend ? Endless.getInstance().getThemeManager().getClientColor(0).getRGB() : Endless.getInstance().getThemeManager().getCurrentTheme().getFriendColor().getRGB();
        int client2 = !friend ? Endless.getInstance().getThemeManager().getClientColor(90).getRGB() : Endless.getInstance().getThemeManager().getCurrentTheme().getFriendColor().getRGB();
        int client3 = !friend ? Endless.getInstance().getThemeManager().getClientColor(180).getRGB() : Endless.getInstance().getThemeManager().getCurrentTheme().getFriendColor().getRGB();
        int client4 = !friend ? Endless.getInstance().getThemeManager().getClientColor(270).getRGB() : Endless.getInstance().getThemeManager().getCurrentTheme().getFriendColor().getRGB();
