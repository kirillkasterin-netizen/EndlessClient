...
import endless.ere.client.screens.autobuy.items.AutoInventoryItem;
...
import static endless.ere.client.screens.autobuy.items.AutoInventoryBuyScreen.createItem;
...
public final class AutoSbor extends Module {
    ...
    private ArrayList<ItemBuy> getLib(){
        ArrayList<ItemBuy> allItemBuys = new ArrayList<>(funtimeMode.isSelected()?Endless.getInstance().getAutoBuyManager().getFuntime():Endless.getInstance().getAutoBuyManager().getHollyworld());
        allItemBuys.addAll(Endless.getInstance().getAutoBuyManager().getVanilla());
        return allItemBuys;
    }
    @EventTarget
