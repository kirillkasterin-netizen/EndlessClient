...
import endless.ere.base.events.impl.other.EventWindowResize;
...
import endless.ere.client.hud.elements.component.*;
...
import endless.ere.utility.render.display.base.GuiUtil;
import endless.ere.utility.render.display.base.color.ColorRGBA;

import java.util.*;

import static endless.ere.utility.render.display.Render2DUtil.glowCache;
...
public final class Interface extends Module {
    ...
    private final endless.ere.client.modules.api.setting.impl.ModeSetting hudBackground =
            new endless.ere.client.modules.api.setting.impl.ModeSetting("Фон HUD", "Сетка", "Точки", "Нет");
    ...
    {
        // Прячем неиспользуемые настройки из меню
    ...
    private Interface() {
        ...
        Endless.getInstance().getNotifyManager().setNotifyComponent(notifyComponent);

        addElement(new KeybindsComponent("Keybinds", 349.0f, 0.0f, 960.0f, 495.5f, -122.0f, 73.0f, DraggableHudElement.Align.TOP_RIGHT));      // 4 - Бинды
        addElement(new TargetHudComponent("TargetHUD", 166.5f, 128.5f, 960.0f, 495.5f, 0.0f, 31.75f, DraggableHudElement.Align.CENTER));       // 5 - Таргет худ
        addElement(new NearbyPlayersComponent("NearbyPlayers", 0.0f, 0.0f, 960.0f, 495.5f, -10.0f, 73.0f, DraggableHudElement.Align.TOP_RIGHT)); // 6 - Игроки рядом
    ...
    public void onRender(EventHudRender event) {
        ...
        float width = mc.getWindow().getFramebufferWidth() / getCustomScale();
        float height = mc.getWindow().getFramebufferHeight() / getCustomScale();
    ...
    public String getHudBackground() {
        return hudBackground.get();
    }
    @EventTarget
