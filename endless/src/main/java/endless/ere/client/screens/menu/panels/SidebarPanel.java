...
public class SidebarPanel {
    ...
    public void render(UIContext ctx, float boxX, float boxY, float height, float progress, Theme theme, Category selectedCategory, ColorRGBA primary, ColorRGBA textColor, ColorRGBA selectedColor) {
        ...
        ctx.drawText(Fonts.ICONS.getFont(11), "5", logoX + 2, logoY + 3, Endless.getInstance().getThemeManager().getColorCycleIcon().toGradient().mulAlpha(progress));
        ...
        DrawUtil.drawRoundedTexture(ctx.getMatrices(), Endless.id("icons/avatar.png"), avatarX, avatarY, avatarSize, avatarSize, BorderRadius.all(4), ColorRGBA.WHITE.mulAlpha(progress));
