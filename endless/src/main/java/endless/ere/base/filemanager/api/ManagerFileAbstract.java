package endless.ere.base.filemanager.api;
...
public class ManagerFileAbstract<T> {
    ...
    public ManagerFileAbstract(String fileName, String shifr, Type type, Supplier<Collection<T>> collectionSupplier) {
        ...
        File file = new File(Endless.DIRECTORY, fileName);
    ...
    public void save() {
        ...
        try (FileWriter writer = new FileWriter(new File(Endless.DIRECTORY, fileName))) {
    ...
    public void load() {
        try (BufferedReader reader = new BufferedReader(new FileReader(new File(Endless.DIRECTORY, fileName)))) {
            Gson gson = new Gson();
