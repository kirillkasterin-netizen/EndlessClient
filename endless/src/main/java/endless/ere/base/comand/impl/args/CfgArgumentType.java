...
public class CfgArgumentType implements ArgumentType<String> {
    ...
    public String parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (Endless.getInstance().getConfigManager().findConfig(config)==null) throw new DynamicCommandExceptionType(
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(Endless.getInstance().getConfigManager().configNames(), builder);
    }
