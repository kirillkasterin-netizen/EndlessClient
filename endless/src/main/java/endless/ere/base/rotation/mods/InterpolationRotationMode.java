...
public class InterpolationRotationMode extends RotationMode {
    ...
    public Rotation process(InterpolationRotationConfig config, Rotation modelOut, Rotation targetRotation
        ...
        Pair<Float, Float> pair = calculateFactors(endless.getRotationManager().getCurrentRotation(), targetRotation,config.getHorizontalSpeedSetting(),config.getVerticalSpeedSetting(),config.getDirectionChangeFactor(),config.getMidPoint());
    ...
    public Pair<Float, Float> calculateFactors(Rotation currentRotation, Rotation targetRotation, IntRange horizontalSpeedSetting, IntRange verticalSpeedSetting, IntRange directionChangeFactor, float midpoint) {
        ...
        if (targetRotation != null && endless.getRotationManager().getPreviousRotationTarget() != null) {
            directionChange = endless.getRotationManager().getPreviousRotationTarget().targetRotation()
