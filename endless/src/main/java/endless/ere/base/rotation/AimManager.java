...
import endless.ere.base.rotation.mods.AIRotationMode;
import endless.ere.base.rotation.mods.InstantRotationMode;
import endless.ere.base.rotation.mods.InterpolationRotationMode;
import endless.ere.base.rotation.mods.config.AiRotationConfig;
import endless.ere.base.rotation.mods.config.InstantRotationConfig;
import endless.ere.base.rotation.mods.config.InterpolationRotationConfig;
...
public class AimManager implements IClient {
    ...
    public Rotation rotate(RotationConfig config, Rotation targetRotation) {
        if (config.getType() != RotationModeType.INSTANT) {
            RotationDelta deltaToTarget = endless.getRotationManager().getCurrentRotation().rotationDeltaTo(targetRotation);
        ...
        switch (config.getType()) {
            ...
            case INTERPOLATION -> newRotation = interpolationMod.process((InterpolationRotationConfig) config,endless.getRotationManager().getCurrentRotation(), targetRotation);
            ...
            default -> newRotation = endless.getRotationManager().getCurrentRotation();
        }
