...
import endless.ere.base.rotation.deeplearnig.MinaraiModel;
...
public class AIRotationMode extends RotationMode {
    ...
    public Rotation process(AiRotationConfig config, Rotation targetRotation) {
        ...
        RotationDelta prevDelta = endless.getRotationManager().getPreviousRotation().rotationDeltaTo(endless.getRotationManager().getCurrentRotation());

        Rotation currentRotation = endless.getRotationManager().getCurrentRotation();
    ...
    private Rotation process(AiRotationConfig config, Rotation currentRotation,Rotation targetRotation,RotationDelta prevDelta, boolean tickUpdate) {
        ...
        MinaraiModel model = endless.getDeepLearningManager().getSlowModel();
        try {
