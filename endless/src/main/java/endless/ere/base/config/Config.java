...
public class Config {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(ConfigManager.configDirectory, name + "." + Endless.NAME.toLowerCase());
    ...
    public JsonObject save() {
        try {
            ...
            for (Module module : Endless.getInstance().getModuleManager().getModules()) {
                modulesObject.add(module.getName(), module.save());
            }
            root.add("Modules", modulesObject);

            
            ThemeManager themeManager = Endless.getInstance().getThemeManager();

            JsonObject themeObject = new JsonObject();
            themeObject.addProperty("selected", themeManager.getCurrentTheme().getName());
            themeObject.addProperty("columns", Endless.getInstance().getMenuScreen().getColumns());
    ...
    public void load(JsonObject object) {
        ...
        if (object.has("Theme")) {
            ...
            if (themeObject.has("selected")) {
                ...
                Endless.getInstance().getThemeManager().switchThemeByName(selected);
            }
            if (themeObject.has("columns")) {
                int columns = themeObject.get("columns").getAsInt();
                Endless.getInstance().getMenuScreen().setColumns(columns);
            }
