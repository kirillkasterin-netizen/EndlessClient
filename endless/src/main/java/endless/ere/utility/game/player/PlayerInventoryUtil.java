package endless.ere.utility.game.player;
...
import endless.ere.client.hud.elements.component.CooldownComponent;
...
public class PlayerInventoryUtil implements IClient {
    ...
    public void swapAndUse(Item item) {
        swapAndUse(item, Endless.getInstance().getRotationManager().getCurrentRotation());
    ...
    public void swapAndUse(Item item, Rotation angle) {
        ...
        if (cooldownProgress > 0) {
           ...
           // Notifications.getInstance().addList(Formatting.RED + item.getName().getString() + Formatting.RESET + " - в кд еще " + time, 2000);
            Endless.getInstance().getNotifyManager().addNotification( "N",Text.of(item.getName().copy().setStyle(Style.EMPTY.withColor(endless.getThemeManager().getCurrentTheme().getColor().getRGB())).append(Text.of("находиться в кд").copy().setStyle(Style.EMPTY.withColor(endless.getThemeManager().getCurrentTheme().getWhite().getRGB())))));
            return;
        }
        ...
        if (slot == null) {
          //  Notifications.getInstance().addList(Formatting.RED + item.getName().getString() + Formatting.RESET + " - не найден!", 2000);
            Endless.getInstance().getNotifyManager().addNotification( "M",Text.of(item.getName().copy().setStyle(Style.EMPTY.withColor(endless.getThemeManager().getCurrentTheme().getColor().getRGB())).append(Text.of("не найден").copy().setStyle(Style.EMPTY.withColor(endless.getThemeManager().getCurrentTheme().getWhite().getRGB())))));
    ...
    public void swapAndUse(Slot slot, Rotation angle) {
        // GrimAC-friendly silent swap:
        // если предмет уже в хотбаре - просто меняем selectedSlot через UpdateSelectedSlotC2SPacket,
        // используем, и возвращаем обратно. Без ClickSlot(SWAP)/CloseScreen которые Grim ловит как inventory desync.
        int hotbarSlot = slot.id - (mc.player.currentScreenHandler.slots.size() - 9);
        boolean isHotbar = hotbarSlot >= 0 && hotbarSlot <= 8 && !isServerScreen();
        ...
        // fallback (предмет не в хотбаре) - старая схема через SWAP
