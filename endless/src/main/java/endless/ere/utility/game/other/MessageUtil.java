package endless.ere.utility.game.other;
...
public class MessageUtil implements IMinecraft {
    ...
    public static final Text PREFIX = Text.of("[ %s ] ".formatted(Endless.NAME))
            .copy()
            .append(Text.of("★"))
    ...
    public void displayMessage(LogLevel level, String message) {
        ...
        Text icon = Text.of("[ %s ] ".formatted(Endless.NAME))
                .copy()
                .append(Text.of("★"))
                .getWithStyle(Style.EMPTY.withColor(Endless.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB()))
                .getFirst();
