...
public class IdealHitUtils implements IMinecraft {
    ...
    public boolean canAIFall() {
        return ((getBlock(0, 3, 0) == Blocks.AIR && getBlock(0, 2, 0) == Blocks.AIR && getBlock(0, 1, 0) == Blocks.AIR)
                || Endless.getInstance().getServerManager().getFallDistance() < (getBlock(0, 2, 0) != Blocks.AIR ? 0.08f : 0.6f)
                || Endless.getInstance().getServerManager().getFallDistance() > 1.2f);
    }
    ...
    public boolean canCritical() {
        ...
        boolean notCrit = mc.player.isInLava()
                ...
                || Endless.getInstance().getModuleStorage().get(FreeCamera.class).isEnabled()
                || mc.player.isOnGround() && !mc.options.jumpKey.isPressed() && !Endless.getInstance().getModuleStorage().get(KillAura.class).onlySpace.getValue();
