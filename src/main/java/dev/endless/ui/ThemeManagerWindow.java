...
public class ThemeManagerWindow implements IMinecraft {
    ...
    private static final File THEME_DIR = new File("endless");
    ...
    private void applyColors() {
        dev.endless.module.settings.impl.Theme t =
            dev.endless.module.settings.impl.ThemeManager.getInstance().getCurrentTheme();
        int[] c = currentColors;
    ...
    // ───────────────────── API для нижнего тулбара выбора темы ──────────────────────

    public int getPresetCount() {
        return PRESETS.length + savedThemes.size();
    }

    public int getPresetAccent(int idx) {
        if (idx < 0) return 0;
        if (idx < PRESETS.length) return PRESETS[idx][0];
        return savedThemes.get(idx - PRESETS.length).colors[1];
    }

    public boolean isPresetActive(int idx) {
        if (idx < 0 || idx >= getPresetCount()) return false;
        if (idx < PRESETS.length) {
            int[] p = PRESETS[idx];
            return currentColors[1] == p[0] && currentColors[0] == p[1];
        }
        SavedTheme st = savedThemes.get(idx - PRESETS.length);
        for (int i = 0; i < COLOR_COUNT; i++) {
            if (currentColors[i] != st.colors[i]) return false;
        }
        return true;
    }

    public void applyPresetByIndex(int idx) {
        if (idx < 0 || idx >= getPresetCount()) return;
        if (idx < PRESETS.length) {
            int[] p = PRESETS[idx];
            currentColors = buildDefaultColors(p[0], p[1]);
        } else {
            currentColors = savedThemes.get(idx - PRESETS.length).colors.clone();
        }
        applyColors();
        save();
    }
